class CodeGenerator(object):
    def __init__(self, codeEmitter, useSpim):
        ...
        self.condCounter = 0
        # Contador para genetar lebels unicos para la multiplicacion
        self.multCounter = 0
        # Contador para generar labels unicos para la division
        self.divCounter = 0
    ...
    def mulexpr(self, tree):
        ...
        if op == "*":
            ...
            self.ce.blt("t1", "a0", "true_" + str(self.condCounter))
            #Cuando es falso
            self.ce.newLabel("false_" + str(self.condCounter))
            self.ce.move("t2", "a0")
            self.ce.move("a0", "t1")
            self.ce.jal("mult_" + str(self.multCounter))
            #Cuando es verdadero
            self.ce.newLabel("true_" + str(self.condCounter))
            self.ce.move("t2", "t1")
            #Multiplicar
            self.ce.li("t3",0)
            self.ce.newLabel("mult_" + str(self.multCounter))      
            self.ce.beq("t2","zero","exit_" + str(self.condCounter))
            self.ce.add("t3","t3","a0")     
            self.ce.addiu("t2","t2",-1)
            self.ce.jal("mult_" + str(self.multCounter))

            #Termina
            self.ce.newLabel("exit_" + str(self.condCounter))
            self.ce.move("a0","t3")
            self.ce.move("t3","zero")
        elif op == "/":
            self.condCounter = self.condCounter + 1
            self.divCounter = self.divCounter + 1
            self.ce.beq("a0", "zero", "exit_" + str(self.condCounter))
            self.ce.li("t2", 0)
            self.ce.newLabel("div_" + str(self.divCounter))
            self.ce.sub("t1","t1","a0")
            self.condCounter = self.condCounter + 1
            self.ce.blt("t1","zero","exit_" + str(self.condCounter))
            self.ce.addiu("t2","t2",1)
            self.ce.jal("div_" + str(self.divCounter))

            self.ce.newLabel("exit_" + str(self.condCounter))
            self.ce.add("a1","t1","a0")
            self.ce.move("a0","t2")
            self.ce.move("t2","zero")

            self.ce.newLabel("exit_" + str(self.condCounter-1))
            self.ce.move("zero","zero")
        else:
            assert op == "%"
            self.condCounter = self.condCounter + 1
            self.divCounter = self.divCounter + 1
